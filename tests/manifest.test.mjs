...
test("template manifest keeps the Vera Solaro source identity and market defaults", () => {
  assert.equal(packageJson.name, "@astropages/aspt-midnight-ledger-vera-solaro");
  assert.equal(packageJson.emdash.label, "Vera Solaro");
  assert.equal(manifest.templateKey, "aspt-midnight-ledger-vera-solaro");
  ...
  assert.deepEqual(manifest.repo, {
    url: "https://github.com/vivekrto9/aspt-midnight-ledger-vera-solaro.git",
    commitSha: "RESOLVED_AT_TEMPLATE_REGISTRATION",
  });
  assert.equal(
    manifest.secrets.deploymentMapping.workerSecretsFile,
    "aspt-midnight-ledger-vera-solaro-worker-secrets.json",
  );
  ...
  assert.equal(generatedSettings.siteSettings.primaryColor, "#B0A2FF");
  assert.equal(generatedSettings.siteSettings.tone, "midnight-ledger");
