...
test("Midnight Ledger anchors remain the public content defaults", async () => {
  const {
    homeDefaults,
  ...
  assert.equal(homeSectionsDefaults.reading_1_title, "The Natal Hour");
  assert.equal(homeSectionsDefaults.reading_1_meta, "30 minutes · in person or by call");
...
test("generated EmDash types retain every Studio collection and the Ledger home fields", () => {
  const types = read("emdash-env.d.ts");
  const homeSection = types.match(/export interface VeraHomeSection \{[\s\S]*?\n\}/)?.[0] ?? "";
  ...
  for (const collection of [
    ...
    assert.match(types, new RegExp(`\\b${collection}:`));
  }
  for (const field of [
    "sitting_step_5_time",
...
test("account access forms validate fields locally and name every failure in Studio copy", async () => {
  ...
  // author a sentence, which is what keeps backend wording out of the public pages.
...
test("source media stays in registered library slots and the shared article stand-in", () => {
  ...
  // the owner to place, which is why the Midnight Ledger home keeps explicit placeholders.
  ...
  assert.match(home, /after_\$\{index\}_alt/);
  assert.match(home, /const deliverables = \[1, 2, 3\]/);
  assert.doesNotMatch(home, /aliases\/vera-portrait\//);
  assert.doesNotMatch(authorized, /aliases\/vera-portrait\//);
});

test("the home follows the Midnight Ledger order and exposes one offered reading", () => {
  const home = read("src/pages/index.astro");
  ...
  for (const anchor of orderedAnchors) {
    const current = home.indexOf(anchor);
    assert.ok(current > previous, `${anchor} must follow the source section order`);
    previous = current;
  }

  assert.match(home, /const offeredReadings = \[\{ index: 1, slug: "natal-hour" \}\]/);
  assert.doesNotMatch(home, /reading_2_|reading_3_|data-newsletter-form|vera-journal|home_about/);
  assert.match(styles, /--ledger-night:\s*#070a1a/);
  assert.match(styles, /--ledger-violet:\s*#b0a2ff/);
  assert.match(styles, /--ledger-mint:\s*#6fd1a8/);
...
test("source-authorized reading, letter, writing, and closed screens stay reachable", async () => {
  const {
    aboutDefaults,
    chromeDefaults,
    contactDefaults,
...
test("Vera typography has no remote font stylesheet", () => {
  ...
  assert.match(frame, /@fontsource\/righteous\/400\.css/);
  assert.match(frame, /@fontsource\/spectral\/300-italic\.css/);
  assert.match(frame, /@fontsource\/ibm-plex-mono\/500\.css/);
  assert.doesNotMatch(frame, /shrikhand|libre-baskerville|barlow-condensed/);
});
